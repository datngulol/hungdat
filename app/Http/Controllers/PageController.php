...
class PageController extends Controller
{
     ...
     public function postReservation(Request $request)
    {
        ...
        if (count($room)>0) 
        {
            ...
            $reservation->status = 0; // Gán giá trị cho trường status
            $reservation->save();   
            ...
            $bill->save();  
            return redirect('reservation/{id}')->with('annoucement','Đặt chỗ thành công.Phòng của bạn là '.$roomtaken[0]->name .'  .See you soon !');
        }
        else return redirect('reservation/{id}')->with('annoucement','Loại phòng bạn đặt đã hết. Vui lòng tham khảo các loại phòng còn lại trong hệ thống khách sạn. Xin cảm ơn !');
    }
}
